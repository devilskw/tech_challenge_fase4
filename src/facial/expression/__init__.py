...
class FaceDetector:
    ...
    # def __get_codec__(self, codec_code) -> str:
    #     return "".join([chr((int(codec_code) >> 8 * i) & 0xFF) for i in range(4)])
    ...
    def __analyse_expressions__(self, frame, id_frame):
        try:
            ...
            for face in faces:
                ...
                if id_frame % 25 == 0: # TODO SOMENTE PARA FINS DO TRABALHO (SE GERAR MUITAS IMAGENS A GENTE NÃO DEVE FAZER ISSO, OU RESTRINGIR PARA GERAR AS 'N' PRIMEIRAS IMAGENS SOMENTE, PARA VALIDAR)
